...
//             image.setColor(Color.BLACK);
            ...
            if (active.startsWith(barItem.getName() + "/"))
            {
                ...
                while (!s.isEmpty() || sneakpeek)
                {
                    ...
                    for (int i = 0; i < item.getItems().size(); i++)
                    {
                        ...
                        // Draw MenuItem text.
                        image.setColor(!item.getItems().get(i).isEnabled() ? disableColor : textColor);
                        image.drawString(item.getItems().get(i).getName(), runningX + 2 + (item.getItems().get(i).isMemberOfSet() ? checkMarkWidth : 0), runningY + i * font.getSize() + font.getSize() - 1);
                        if (item.getItems().get(i).getShortCut() != null)
                            image.drawString(item.getItems().get(i).getShortCut(), runningX + w - item.getItems().get(i).getWidthOfShortCut() - 2, runningY + i * font.getSize() + font.getSize() - 1);
            ...
            // Frame.
            image.setColor(borderColor);
            image.drawRect(runWidth, 0, barItem.getWidth(), font.getSize());
            // Text.
            image.setColor(textColor);
            image.drawString(barItem.getName(), runWidth + 2, font.getSize() - 1);
