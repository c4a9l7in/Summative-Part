...
public class TextBox extends WindowComponent
{
    ...
    public TextBox(Point size, String text)
    {
        ...
        if (size.getY() < fontHeight + 6)
            size = new Point((int)size.getX(), fontHeight + 6);
        this.size = size;
        this.text = text;
        caret = text.length();
    ...
    private void handleMouse()
    {
        ...
        if (Greenfoot.mousePressed(this))
            ...
            if (mouse.getX() - (getX() - size.getX() / 2) < size.getX() - (strs.size() * fontWidth > size.getY() ? SCROLL_WIDTH - 3 : 0))
            {
                if (focusable) {
            ...
            {
                int scroll = (int)(scrollValue / (double)(fontWidth * strs.size() + fontWidth / 5 - (int)size.getY() + 3) * (size.getY() - scrollBar));
                // Pressed on scroll bar.
                if (strs.size() * fontWidth > (int)size.getY() && mouse.getX() - (getX() - size.getX() / 2) > (int)size.getX() - 15 && mouse.getY() - (getY() - (int)size.getY() / 2) > scroll && mouse.getY() - (getY() - (int)size.getY() / 2) < scroll + scrollBar)
                {
        ...
        if (Greenfoot.mouseDragged(null))
            if (draggingScrollBar)
            {
                ...
                scrollValue = (int)(topOfScrollBar / (double)(size.getY() - scrollBar) * (fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3));
            }
    }
    ...
    {
        GreenfootImage g = new GreenfootImage(1, 1);
        g.setFont(font);
                ...
                if (caret >= strs.get(0).length())
                {
                    ...
                    for (int i = 0; i < strs.size(); i++)
                    {
                        if (l + strs.get(i).length() > caret)
                        {
                            ...
                            if (lastCursorX == null)
                            {
                                cur = 4 + (strs.get(i).substring(0, caret - l)).length()*12;
                                lastCursorX = cur;
                            }
                            else
                                cur = lastCursorX;
                            clickedAt = new Point(cur, i * fontHeight - scrollValue);
                            handleForSelecting();
                ...
                for (int i = 0; i < strs.size(); i++)
                {
                    if (l + strs.get(i).length() > caret)
                    {
                        ...
                        if (lastCursorX == null)
                        {
                            ...
                            cur = lastCursorX;
                        clickedAt = new Point(cur, (i + 2) * fontHeight - scrollValue - 2);
                        handleForSelecting();
                        blink = 1;
                        break;
                    }
...
//     }
    ...
    private class DrawFunction
    {
        ...
        public void paint()
        {
            ...
            while (!remainingBeforeN.isEmpty() || !remainingAfterN.isEmpty())
            {
                ...
                while (!remainingBeforeN.isEmpty())
                {
                    ...
                    int currentWidth = current.length()*fontWidth;
                    ...
                    int nextWidth = next.length()*fontWidth;
                    ...
                    if (currentWidth + nextWidth > size.getX() - SCROLL_WIDTH - 3)
                    {
                        if (current.isEmpty())
                            ...
                            {
                                if ((remainingBeforeN.substring(0, i)).length()*fontWidth > size.getX() - SCROLL_WIDTH - 3)
                                {
                    ...
                    if (needToUpdateRemaining)
                        ...
                        if (current.isEmpty() && nextWidth > size.getX() - SCROLL_WIDTH - 3)
                        {
                            for (int i = 1; i <= next.length(); i++)
                            {
                                if ((next.substring(0, i)).length()*12 > size.getX() - SCROLL_WIDTH - 3)
                                {
            ...
            if (oldCaret != caret && caretLine != -1)
            {
                ...
                int y = fontHeight * (caretLine - 1) + fontHeight / 5 - scrollValue - 3;
                
                // If above top.
                if (y < 0)
                    scrollValue = fontHeight * (caretLine - 1) + fontHeight / 5 - 3;
                // If below bottom.
                else if (y + fontHeight + 6 > size.getY())
                    scrollValue = fontHeight * (caretLine - 1) + fontHeight / 5 + fontHeight - (int)size.getY() + 3;
            }
            ...
            if (strs.size() * fontHeight > size.getY())
            {
                ...
                if (mouseOverThis() && getWorld() != null)
                {
                    ...
                    if (scrollValue < 0)
                        scrollValue = 0;
                    else if (scrollValue > fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3)
                        scrollValue = fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3;
                }
                
                // Y-coordinate relatvie to image, of where top of scroll bar will be.
                int scroll = (int)(scrollValue / (double)(fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3) * (size.getY() - scrollBar));
            ...
            // Adjust scroll amount if out of bounds.
            if (scrollValue < 0 || strs.size() * fontHeight <= size.getY())
                scrollValue = 0;
            else if (scrollValue > fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3)
                scrollValue = fontHeight * strs.size() + fontHeight / 5 - (int)size.getY() + 3;
            ...
            for (int i = 0; i < strs.size(); i++)
                pic.drawString(strs.get(i), 4, fontHeight * (i + 1) - scrollValue);
            ...
            if (blink > 0 && hasFocus())
                if (caretLine != -1 && caretX > 3)
                {
                    ...
                    pic.fillRect(caretX, fontHeight * (caretLine - 1) + fontHeight / 5 - scrollValue, 2, fontHeight);
                }
                // If no text, has focus, draw caret bar blinking animation.
                else if (text.isEmpty())
                {
                    pic.setColor(blinkColor);
                    pic.fillRect(4, fontHeight / 5 + 1, 2, fontHeight);
                }
        ...
        private void addCurrent_drawHighlight_rememberCaret()
        {
            ...
            if (selectingPivot != -1)
            {
                ...
                if (maxC > count && minC < count + current.length())
                {
                    ...
                    if (maxC > current.length())
                        maxC = current.length();
                    int hBegin = (current.substring(0, minC)).length()*12 + 4;
                    int hLength = (current.substring(minC, maxC)).length()*12;
                    pic.setColor(selectColor);
                    pic.fillRect(hBegin, fontHeight * (strs.size() - 1) + fontHeight / 5 - scrollValue, hLength, fontHeight);
                }
            }
            ...
            {
                caretX = (current.substring(0, caret - count)).length()*12 + 4;
                caretLine = strs.size();
            }
        }
        ...
        private void handleCaret_finishCurrent(boolean showingMessage)
        {
            if (!showingMessage)
                ...
                if (clickedAt != null && clickedAt.getY() <= fontHeight * (strs.size() - 1) - scrollValue)
                {
                    caret = 0;
                    clickedAt = null;
                ...
                else if (clickedAt != null && clickedAt.getY() > fontHeight * (strs.size() - 1) - scrollValue && clickedAt.getY() <= fontHeight * strs.size() - scrollValue)
                {
                    // If to the left of entire line of text.
                    if (clickedAt.getX() < 4 + (current.substring(0, 1)).length()*12 / 2)
                    {
                        caret = count;
                        clickedAt = null;
                    }
                    else
                    {
                        ...
                        for (int i = 1; i < current.length() - 1; i++)
                            ...
                            if (clickedAt.getX() >= 4 + ((current.substring(0, i - 1)).length()*12 + (current.substring(i - 1, i)).length()*12) / 2 && clickedAt.getX() < 4 + ((current.substring(0, i)).length()*12 + current.substring(i, i + 1).length()*12) / 2)
                            {
